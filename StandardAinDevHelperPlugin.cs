    ...
    public abstract class StandardAinDevHelperPlugin : IAinDevHelperPlugin {
        ...
        public AinDevHelperPluginActionResult GetSuccessfulResponseWebLinkAction(AinDevHelperPluginAction actionToRun, AinDevHelperPluginWebLinkAction webLinkAction) {            
            return new AinDevHelperPluginActionResult(
                this,
                actionToRun,
                $"Действие '{webLinkAction.GetLocalizedName()}' успешно выполнено. Страница '{webLinkAction.WebLink}' открыта в браузере по умолчанию.",
                (EN, $"Action '{webLinkAction.GetLocalizedName()}' completed successfully. The '{webLinkAction.WebLink}' page is opened in the default browser."),
                (DE, $"Aktion „{webLinkAction.GetLocalizedName()}“ erfolgreich abgeschlossen. Die Seite „{webLinkAction.WebLink}“ wird im Standardbrowser geöffnet.")
            );
        }
